...
void odd_find(Node* root,int i,int& j,double& med)
{
   if(root==NULL) return;
   odd_find(root->left,i,j,med);
   if(i==j) {med=(double)root->data; j++; return;}
   j++;
   odd_find(root->right,i,j,med);
}

void even_find(Node* root,int i,int& j,double& med)
{
   if(root==NULL) return;
   even_find(root->left,i,j,med);
   
   if(i==j) {med=(double)root->data;}
   else if(i+1==j) {med=(med+(double)root->data)/2; j++;return;}
...
double find_median(Node* root)
{
    ...
    int i=n/2,j=0;
    double med=0;
    if(n%2==0) {j++;even_find(root,i,j,med);}
